...
class TransactionManager(object):
    ...
    def add_transaction(self, transaction):
        ...
        for item in transaction:
            if item not in self._transaction_index_map:
                self._items.append(item)
                self._transaction_index_map[item] = set()
            self._transaction_index_map[item].add(self._num_transaction)
        self._num_transaction += 1
    ...
    def calc_support(self, items):
        ...
        sum_indexes = None
        for item in items:
            indexes = self._transaction_index_map.get(item)
            if indexes is None:
        ...
        return len(sum_indexes), float(len(sum_indexes)) / self._num_transaction
...
def gen_support_records(transaction_manager, min_support, **kwargs):
    ...
    while candidates:
        ...
        for relation_candidate in candidates:
            count, support = transaction_manager.calc_support(relation_candidate)
...
def gen_ordered_statistics(transaction_manager, record):
    ...
    for combination_set in combinations(sorted(items), len(items) - 1):
        ...
        count, support = transaction_manager.calc_support(items_base)
        confidence = (
                record.support / support)
        count, support = transaction_manager.calc_support(items_add)
